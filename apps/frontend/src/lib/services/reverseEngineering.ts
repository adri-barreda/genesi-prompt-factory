...
const SUPPORTED_MODES = ['variables', 'analysis'] as const;
type ReverseEngineeringMode = (typeof SUPPORTED_MODES)[number];

function normalizeMode(rawMode?: string): ReverseEngineeringMode {
  if (!rawMode) {
    return 'variables';
  }

  const lower = rawMode.toLowerCase();
  if ((SUPPORTED_MODES as readonly string[]).includes(lower)) {
    return lower as ReverseEngineeringMode;
  }

  throw new Error(`Modo de reverse engineering no soportado: ${rawMode}`);
}

type PromptVariable = z.infer<typeof variableSchema> & {
  prompt_text: string;
  sample_outputs: string[];
};

export async function reverseEngineerEmailVariables(
  emailBody: string,
  { language = 'es-ES', mode }: { language?: string; mode?: string } = {}
) {
...
Devuelve solo el JSON solicitado.`;
  ...
  const parsedByPlaceholder = new Map<string, z.infer<typeof variableSchema>>();
  ...
  const orderedVariables: PromptVariable[] = [];
